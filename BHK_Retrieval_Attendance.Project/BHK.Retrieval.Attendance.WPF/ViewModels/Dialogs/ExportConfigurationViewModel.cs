...
namespace BHK.Retrieval.Attendance.WPF.ViewModels.Dialogs
{
    ...
    public partial class ExportConfigurationViewModel : ObservableObject
    {
        ...
        private void Export()
        {
            try
            {
                ...
                if (saveFileDialog.ShowDialog() == true)
                {
                    ...
                    MessageBox.Show($"Đã xuất {RecordCount} bản ghi vào file:\n{filePath}", 
                                    "Thành công", 
                                    MessageBoxButton.OK, 
        ...
        private void ExportToText(string filePath)
        {
            ...
            writer.WriteLine("DN\tDIN\tNgày\tGiờ\tLoại\tPhương thức\tHành động\tGhi chú");
            writer.WriteLine(new string('-', 80));
            foreach (var record in _data)
            {
                writer.WriteLine($"{record.DN}\t{record.DIN}\t{record.Date}\t{record.Time}\t{record.Type}\t{record.Verify}\t{record.Action}\t{record.Remark}");
            }
        }
